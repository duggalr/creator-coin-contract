...
# # def test_success_safe_mint_with_gas_fee(token, accounts):
# #   """
# #   Success case where 1 NFT is safe minted, with gas fee
# #   """
# #   from_account = accounts[2]
# #   from_account_initial_balance = from_account.balance()
...
# #   eth_price = 0.1
# #   from_account_value_amount = ( eth_price * (10**19) ) * mint_amount
# #   token.safeMint(mint_amount, {'value': from_account_value_amount, 'from': from_account, 'gas_price': 500000})
# #   platform_fee = ( 3 * (eth_price * (10**19)) ) / 100
# #   deployer_amount = from_account_value_amount - platform_fee
  
# #   print(f'deployer-account-balance: {deployer_account.balance()} / from-account-balance: {from_account.balance()} / platform-account-balance: {platform_account.balance()}')

# #   print(f'current-token-ID: {token.getCurrentTokenID()}')

# #   # assert token.getCurrentTokenID() == (initial_token_id + mint_amount)
# #   # assert token.tokenURI( initial_token_id + mint_amount ) == "https://docs.soliditylang.org/"
